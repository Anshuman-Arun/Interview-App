...
export const oxfordEulerLocallyBalancedLabelsSpec = makeEulerCandidateSpec({
  ...
  "givenInformation":["For the path version, use a finite path with at least two vertices.","For a path, the two endpoint labels are not constrained by the averaging rule.","For the cycle version, use a simple cycle with at least three vertices; every vertex has exactly two cyclic neighbors."],
