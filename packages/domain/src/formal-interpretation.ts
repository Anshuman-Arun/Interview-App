...
export const MAX_FORMAL_INTERPRETATION_ID_CHARACTERS = 256 as const;
export const MAX_FORMAL_INTERPRETATION_CLAIM_ID_CHARACTERS = 256 as const;

const BoundedRequestIdSchema = RequestIdSchema.refine(
  (value) => value.length <= MAX_FORMAL_INTERPRETATION_ID_CHARACTERS,
  "Formal interpretation request ID exceeds the bounded length"
);
const BoundedSessionIdSchema = SessionIdSchema.refine(
  (value) => value.length <= MAX_FORMAL_INTERPRETATION_ID_CHARACTERS,
  "Formal interpretation session ID exceeds the bounded length"
);
const BoundedGenerationIdSchema = GenerationIdSchema.refine(
  (value) => value.length <= MAX_FORMAL_INTERPRETATION_ID_CHARACTERS,
  "Formal interpretation generation ID exceeds the bounded length"
);
const BoundedInputEpisodeIdSchema = InputEpisodeIdSchema.refine(
  (value) => value.length <= MAX_FORMAL_INTERPRETATION_ID_CHARACTERS,
  "Formal interpretation InputEpisode ID exceeds the bounded length"
);
const BoundedTurnIdSchema = TurnIdSchema.refine(
  (value) => value.length <= MAX_FORMAL_INTERPRETATION_ID_CHARACTERS,
  "Formal interpretation turn ID exceeds the bounded length"
);
const BoundedEventIdSchema = EventIdSchema.refine(
  (value) => value.length <= MAX_FORMAL_INTERPRETATION_ID_CHARACTERS,
  "Formal interpretation event ID exceeds the bounded length"
);

const FormalInterpretationGenerationBasisSchema = GenerationBasisSchema.superRefine((value, context) => {
  if (value.turnId.length > MAX_FORMAL_INTERPRETATION_ID_CHARACTERS) {
    context.addIssue({ code: "custom", path: ["turnId"], message: "Generation basis turn ID exceeds the bounded length" });
  }
  if (
    value.inputEpisodeId !== undefined
    && value.inputEpisodeId.length > MAX_FORMAL_INTERPRETATION_ID_CHARACTERS
  ) {
    context.addIssue({
      code: "custom",
      path: ["inputEpisodeId"],
      message: "Generation basis InputEpisode ID exceeds the bounded length"
    });
  }
});

export const FormalInterpretationTargetSchema = z.object({
  problemId: z.string().min(1).max(256),
  subject: z.object({
    kind: z.literal("CLAIM"),
    claimId: z.string().min(1).max(MAX_FORMAL_INTERPRETATION_CLAIM_ID_CHARACTERS)
  }).strict(),
  dimension: z.literal("CORRECTNESS")
}).strict();
export type FormalInterpretationTarget = z.infer<typeof FormalInterpretationTargetSchema>;
