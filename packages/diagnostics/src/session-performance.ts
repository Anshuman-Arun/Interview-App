...
export const SessionLatencySummarySchema = z.object({
  ...
  if (summary.count === 0) {
    if (summary.medianMs !== null || summary.slowestMs !== null) {
      context.addIssue({
        code: "custom",
        message: "Empty latency summaries cannot contain measured latency values"
      });
    }
    return;
  }
  if (summary.medianMs === null || summary.slowestMs === null) {
    context.addIssue({
      code: "custom",
      message: "Non-empty latency summaries require median and slowest values"
    });
    return;
  }
  if (summary.medianMs > summary.slowestMs) {
    context.addIssue({
      code: "custom",
      message: "Latency median cannot exceed slowest latency"
    });
  }
});
...
export const SessionPerformanceSummarySchema = z.object({
  ...
  const remoteOutcomeTotal = boundedSum(...Object.values(summary.remote.outcomes));
  if (remoteOutcomeTotal > summary.remote.totalCalls) {
    issue("Remote outcome totals exceed started remote calls");
  }
  if (!summary.partial && remoteOutcomeTotal !== summary.remote.totalCalls) {
    issue("Complete remote metrics require one terminal outcome per remote call");
  }
