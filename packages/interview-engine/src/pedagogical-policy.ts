...
function collectVerificationSignals(
  ...
  const rawEventIds: unknown = state.eventIds;
  if (!Array.isArray(rawEventIds)) {
    return { ok: false, reasonCode: "MALFORMED_POLICY_INPUT" };
  }
  if (rawEventIds.length > MAX_EVENT_IDS) {
    return { ok: false, reasonCode: "RESOURCE_LIMIT_EXCEEDED" };
  }
  if (!rawEventIds.every((eventId) => boundedString(eventId, MAX_POLICY_ID_CHARACTERS))) {
    return { ok: false, reasonCode: "MALFORMED_POLICY_INPUT" };
  }

  const eventSequence = new Map<string, number>();
  rawEventIds.forEach((eventId, index) => {
    eventSequence.set(eventId, index + 1);
  });
