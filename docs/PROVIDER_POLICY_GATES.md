...
Real provider adapters must separately demonstrate the provider-specific enforcement mechanism described by the architecture freeze. The supervised Antigravity path intentionally reports `spendImpossible: false`; disabling CLI credit fallback is not treated as proof that all account-side incremental spend is technically impossible. This generic gate validates evidence; it does not manufacture that evidence.
