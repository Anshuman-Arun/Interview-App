...
The existing coordinator also refuses to choose among multiple distinct admitted formal candidates. Duplicate canonical candidates can collapse; distinct meanings remain ambiguous and produce no deterministic evidence.

## Cancellation and recovery

InterpretationCoordinator.cancel() and abandon() now abort a provider-owned AbortController. The Antigravity adapter passes that signal into the same supervised process runner used by the approved reasoning runtime. Late output remains suppressed by existing current-request and verification-admission checks.
