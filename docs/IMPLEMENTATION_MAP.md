...
apps/server
  ...
  - authoritative interview-session configuration/composition
  - SessionRuntimeRegistry
  - SessionWriter -> SQLite semantic event log
  - ServerTurnOrchestrator
        +--> exact session-bound Oxford problem
        +--> Turn/Context/Disclosure/Provider coordinators
        +--> MockModelAdapter only for mock/default selection
        +--> RendererStreamServer
  - SessionReadService
        +--> grounded evaluation
        +--> replay/history/longitudinal read models
```
...
| Session configuration/composition | `packages/domain/src/session-configuration.ts`, `apps/server/src/interview-session-composition.ts` | `PRODUCTION_WIRED` | Exact mode/target/version and optional registered provider/model identity are persisted, recovered, and validated by the application composition root. |
...
| Typed React/KaTeX interview shell | `apps/web/src/App.tsx`, components/hooks | `PRODUCTION_WIRED` | Default new-session launch still uses the legacy Ramsey compatibility path; session context is server-derived and public-only, and completed/historical sessions expose review UI. |
...
| Grounded session evaluation | `session-evaluator.ts`, `evaluation-model-seam.ts`, `apps/server/src/session-read-service.ts` | `PRODUCTION_WIRED` | Exact-provenance deterministic evaluation is surfaced through authenticated bounded read routes and post-session/historical review UI. |
| Replay/history projections | `packages/replay/src/*`, `apps/web/src/session-read-client.ts` | `PRODUCTION_WIRED` | Timeline/session/longitudinal/provenance read models are surfaced through authenticated read-only product routes; possibly-exposed content remains withheld. |
| Oxford/quant problem catalogs | `packages/problems/src/problem-catalog.ts`, curated modules | `BACKEND_IMPLEMENTED` | Generic server configuration/catalog resolution consumes exact identities; the main UI still lacks selectable mode/problem launch controls. |
...
| Provider control plane | `packages/providers/src/control-plane.ts`, safe configuration/builtin provider modules | `BACKEND_IMPLEMENTED` | Bounded provider/model/configuration/secret-reference machinery; session composition validates registered provider/model identity, while live non-mock execution remains deferred. |
| `MockModelAdapter` | `packages/providers/src/mock-model-adapter.ts` | `PRODUCTION_WIRED` | Explicitly instantiated only for the registered mock/default execution path; configured non-mock selection is not silently substituted. |
| `GeminiApiAdapter` | `packages/providers/src/gemini-api-adapter.ts` | `RUNTIME_SEAM_ONLY` | Gated adapter exists and is tested; its identity is representable in session configuration but it is not the live server reasoning provider. |
...
- uses the legacy Ramsey compatibility command for the default main-UI new-session action even though server/session composition supports exact configured Oxford and quant identities;
- has no live non-mock reasoning provider; mock execution is restricted to the registered mock/default path;
- has no end-to-end voice path exposed in the interview UI;
- does not expose Quant Trading or Quant Research as selectable main-UI modes;
...
- Vitest files matched by the repository naming convention: 109.
