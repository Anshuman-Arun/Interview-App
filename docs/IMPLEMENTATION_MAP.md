...
| Testing infrastructure | `tests/`, `vitest.config.ts`, `scripts/check-architecture-boundaries.mjs`, `.github/workflows/ci.yml` | unit, replay, crash, idempotency, exhaustive verifier, context reproducibility, fixture integrity, provider-policy, redaction, browser transport, and randomized property tests on Windows/Linux CI | all implemented modules | 0 implemented and enforced in CI |
