...
| Context Compiler | `packages/domain/src/context.ts`, `packages/interview-engine/src/context-compiler.ts`, `context-coordinator.ts`, generation event/state/reducer | `compileContext`, `CompiledContext`, canonical SHA-256 hashing, `ContextCompilationManifest`, `ContextCoordinator` | problem public data, policy, GenerationBasis, serialized writer | 0 safe projection and persisted reproducibility manifest implemented |
...
| Provider policy/billing verification | `packages/providers/src/policy.ts`, `execution.ts` | policy/privacy/clock preflight; adapter-owned `verifyBillingSafety`; `assertProviderPermitted`; `openProviderExecutionSession`; fixed policy/execution errors | capabilities, clock, strict billing schema | 0 fail-closed execution gate implemented; real-provider proofs pending |
