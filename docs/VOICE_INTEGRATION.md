...
Barge-in also invalidates the existing renderer-stream connection on both sides: immediately after `beginUtterance()` the server destroys the live session renderer socket as a physical-interruption backstop, and an admitted onset received by the browser aborts/replaces its consumer while directly interrupting/clearing local audio. This means loss of the PCM response cannot leave the old renderer connection as the sole path keeping stale audio alive. The stream reset is also an ordering barrier for commands that may already have been written to the old SSE socket but not yet processed by the browser. The old consumer stops processing buffered delivery blocks after abort; the replacement connection attaches only after the old consumer settles and server-side disconnect classification completes. Cancelled or `POSSIBLY_EXPOSED` output is therefore not replayed.
