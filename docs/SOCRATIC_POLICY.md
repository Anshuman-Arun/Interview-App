...
Only assistance that reached `EXPOSED`, `COMPLETED`, or `POSSIBLY_EXPOSED` counts as prior disclosed assistance. Cancelled or merely queued content does not. Assistance is tied to the generation-bound action snapshot and exact target, not to a generic "latest step" label. Historical assistance must also correspond to the exact interviewer proposal snapshot present when `PROPOSAL_VALIDATED` was reduced, and to a realization that validated proposal actually contained. The derived validation proof survives legitimate later supersession, but a forged `VALIDATED`/ `SUPERSEDED` status, missing validation snapshot, later unvalidated proposal overwrite, or unrelated exposed atom fails closed.
...
- requires the closed-world analyzer to recognize the complete realization: finding a known protected phrase records that disclosure but does not whitelist arbitrary unreviewed surrounding text;
