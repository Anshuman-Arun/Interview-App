...
The server boxes above are the **current production path**. Session configuration binds exact problem/scenario and provider/model identity. An explicitly selected `antigravity-cli / gemini-3.7-flash-medium` session can reach a real supervised local CLI process and remote Antigravity inference when the host policy opt-in is enabled and the CLI is locally authenticated. Registration alone still does not authorize execution: provider policy, billing/data-use admission, proposal validation, generation compatibility, and delivery remain application-owned. Speech model seams and live vision backends remain separate runtime concerns.
...
| Antigravity CLI reasoning provider | `PRODUCTION_WIRED` | Explicitly configured sessions can execute `gemini-3.7-flash-medium` through a fresh supervised `agy` process per turn. The host must opt into remote reasoning, the CLI must be locally authenticated, and no mock fallback occurs on failure. |
