...
The current production composition is a typed interview shell hosted by the Electron trusted bootstrap and backed by local loopback transport, SQLite event sourcing, real tldraw, application-owned session configuration/composition, provider-controlled Socratic text orchestration, and authenticated read-only evaluation/replay/history review. The main New Interview flow now consumes the server-published interview catalog and provider launch-readiness metadata, allowing exact Oxford problem or Quant scenario selection together with an available provider/model. Explicitly configured sessions can execute through the registered mock path or, on Windows desktop/server hosts, the supervised Antigravity CLI reasoning path; Antigravity remains host-policy gated and requires a locally installed/authenticated CLI. Non-Windows application composition fails closed for that provider because this runtime does not claim hostile-descendant containment from a POSIX process group. The bare browser renderer can be exercised through injected/test seams, but the current `App` has no built-in browser-token acquisition flow.
...
| Quant Trading engine | `PRODUCTION_WIRED` | Deterministic market-making/interview infrastructure is selectable through configured launch and executes through the mode-specific runtime without attaching the Oxford renderer stream. |
| Quant Research engine | `PRODUCTION_WIRED` | Deterministic scenario/evidence/result infrastructure is selectable through configured launch and preserves its mode-specific runtime/recovery path. |
| Curated Oxford + quant problem catalogs | `PRODUCTION_WIRED` | The server publishes exact catalog identities and the New Interview UI exposes only those authoritative mode/target entries. |
...
| Antigravity CLI reasoning provider | `PRODUCTION_WIRED` | Windows-only in current composition. Launch metadata advertises it as available only after policy admission and the real supervised runtime passes the cached exact-version/zero-turn profile preflight. Configured sessions can execute `gemini-3.7-flash-medium` through a fresh Job-Object-supervised `agy` process per turn. Because cached auth may still represent a billable Cloud/project mode, default no-metered policy fails closed; `INTERVIEW_ALLOW_METERED_REMOTE_REASONING=1` is required. No mock fallback occurs on failure; non-Windows hosts fail closed. |
...
As of the 2026-09-03 configured-launch/provider-runtime integration:

1. supervised Antigravity reasoning is available only when the local CLI is installed/authenticated, its exact audited runtime preflight succeeds, and the trusted host explicitly opts into metered-unknown remote reasoning; the restricted runtime profile disables AI-credit fallback and inherited API-key/custom-endpoint configuration but does not claim that cached account/project auth makes spend impossible;
2. there is no automatic provider fallback or escalation; unavailable or failed Antigravity execution fails closed rather than silently switching to mock;
3. live speech/TTS/vision model availability remains determined by their separate runtime composition work;
4. the default `App` still does not provide a standalone browser-token bootstrap.
...
The next product milestone is deeper vertical product integration rather than another broad backend expansion: harden the mode-specific Quant Trading/Quant Research workspaces, continue live voice/vision runtime integration, and improve recovery/readiness UX while preserving the existing event, disclosure, provider-policy, delivery, replay, and security invariants.
