...
describe("deterministic Quant Research interview engine", () => {
  ...
  it("public state and diagnostics omit seed, latent truth, hidden arrays, and exact optima", () => {
    for (const definition of [bayesian, sampling, experimental, model, optimization]) {
      ...
      expect(serialized).not.toMatch(/"(?:hiddenCenter|hiddenPopulation|hiddenMeanA|hiddenMeanB|hiddenModel|hiddenIntercept|hiddenSlope|sequenceA|sequenceB|points|perturbedPoints|baseBestObjective|perturbedBestObjective|sampleOrder|observations|successes)":/u);
    }
  });
  ...
  it("snapshots the authoritative replay definition before action-container traps can mutate caller input", () => {
    ...
    let invalidActionContainerInspected = false;
    const untouchedActions = new Proxy([] as unknown[], {
      getOwnPropertyDescriptor(target, property) {
        invalidActionContainerInspected = true;
        return Reflect.getOwnPropertyDescriptor(target, property);
      }
    });
    expectCode(
      () => replayQuantResearch({ ...bayesian, seed: -1 }, untouchedActions),
      "INVALID_DEFINITION"
    );
    expect(invalidActionContainerInspected).toBe(false);
  });

  it("canonicalizes each replay action before later container traps can rewrite it", () => {
    const firstAction = { actionId: "snapshot-r1", kind: "REQUEST_OBSERVATION", count: 2 };
    const secondAction = { actionId: "snapshot-r2", kind: "SUBMIT_NUMERIC_ESTIMATE", value: 0 };
    const actionTarget: unknown[] = [firstAction, secondAction];
    const maliciousActions = new Proxy(actionTarget, {
      getOwnPropertyDescriptor(target, property) {
        if (property === "1") firstAction.count = 10;
        return Reflect.getOwnPropertyDescriptor(target, property);
      }
    });

    const replayed = replayQuantResearch(sampling, maliciousActions);
    expect(firstAction.count).toBe(10);
    expect(replayed.acceptedActions[0]).toEqual({
      actionId: "snapshot-r1",
      kind: "REQUEST_OBSERVATION",
      count: 2
    });
    expect(replayed.state.stage).toBe("OUTLIER_PERTURBATION");
  });
