...
describe("Agent C — Cantor Wave 2 Oxford candidates", () => {
  it("keeps exactly 17 raw author candidates while exposing only the certified projection live", () => {
    ...
    for (const candidate of oxfordCantorReviewEntries) {
      ...
      expect(liveIds.has(candidate.problem.id)).toBe(true);
      expect(reviewProblemIds.has(candidate.problem.id)).toBe(false);
      expect(reviewMetadataIds.has(candidate.problem.id)).toBe(false);
      expect(getProblemMetadataById(candidate.problem.id)?.reviewStatus).toBe("ready");
    }
  });
