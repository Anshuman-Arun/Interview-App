...
describe("Oxford formal target admission", () => {
  ...
  it("admits only source-grounded target-shaped concrete statements", () => {
    ...
    expect(admitted(
      "oxford-euclid-primes",
      "For listed primes two, three, and five, the product plus one is thirty-one and leaves remainder one modulo two.",
      JSON.stringify({
        protocol: "INTERVIEW_APP_MODULAR_ARITHMETIC_CLAIM",
        protocolVersion: 1,
        claim: {
    ...
    expect(admitted(
      "oxford-prefix-sums-mod-n",
      "For n five, two prefix sums are twelve and seven, so their difference five is divisible by five.",
      JSON.stringify({
        protocol: "INTERVIEW_APP_MODULAR_ARITHMETIC_CLAIM",
        protocolVersion: 1,
        claim: {
  ...
  it("rejects invented numerals and target-shaped trivial substitutions", () => {
    ...
    expect(admitted(
      "oxford-euclid-primes",
      "For the listed prime 2, the constructed product plus one is 31; separately, 3 leaves remainder 1 modulo 2.",
      JSON.stringify({
        protocol: "INTERVIEW_APP_MODULAR_ARITHMETIC_CLAIM",
        protocolVersion: 1,
        claim: {
    ...
    expect(admitted(
      "oxford-divisibility-chain",
      "6 is even; 9 equals 3 times 3 and 24 equals 8 times 3, so 6 divides 24.",
      JSON.stringify({
        protocol: "INTERVIEW_APP_MODULAR_ARITHMETIC_CLAIM",
        protocolVersion: 1,
        claim: {
    ...
    expect(admitted(
      "oxford-divisibility-chain",
      "6 divides 12; separately, 9 and 24 have the same odd part, and 6 divides 24.",
      JSON.stringify({
        protocol: "INTERVIEW_APP_MODULAR_ARITHMETIC_CLAIM",
        protocolVersion: 1,
        claim: {
