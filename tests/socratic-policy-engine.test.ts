...
describe("production Socratic policy engine", () => {
  ...
  it("fails closed when exposed delivery content was never part of its recorded proposal", () => {
    const { state: base, turnId } = makeState();
    let state = withEvidence(
      base,
      milestoneKey(sixPeopleProblem, "choose-vertex", "CORRECTNESS"),
    ...
    state = {
      ...
      deliveries: {
        ...
        [deliveryId]: {
          ...delivery,
          content: { medium: "TEXT", text: "forged text never validated for this generation" }
        }
      }
    };
...
describe("takeover adversarial regressions", () => {
  it("fails closed instead of throwing when verification state has a non-array event ID container", () => {
    const { state, turnId } = makeState();
    ...
    const decision = decidePedagogicalPolicy(malformedState, turnId, sixPeopleProblem);
    expect(decision.reasonCode).toBe("MALFORMED_POLICY_INPUT");
    expect(decision.realizationRequest.maximumDisclosure).toBe(0);
  });

  it("rejects a provider disclosure-level claim above the application ceiling", () => {
    const validator = new DisclosureValidator(new ClosedWorldDisclosureAnalyzer(["safe probe"]));
    const result = validator.validate({
      proposal: {
        realizedAction: "CLARIFY",
    ...
    expect(result.accepted).toBe(false);
    if (!result.accepted) expect(result.reason).toMatch(/claimed.*above|authorized boundary/iu);
  });

  it("does not let a protected substring whitelist otherwise unreviewed text", () => {
    const disclosure = sixPeopleProblem.interviewer.protectedDisclosures[0];
    expect(disclosure).toBeDefined();
    if (disclosure === undefined) throw new Error("missing protected disclosure");
    ...
    const result = validator.validate({
      proposal: {
        ...
        claimedDisclosureLevel: disclosure.minimumDisclosureLevel,
        claimedDisclosureIds: [disclosure.id],
        speechText: disclosure.fact + " This additional realization was never reviewed."
      },
      request: {
        requiredAction: "DIRECTIONAL_NUDGE",
        maximumDisclosure: disclosure.minimumDisclosureLevel,
  ...
  it("allows an explicitly reviewed protected-containing realization while preserving its disclosure metadata", () => {
    const disclosure = sixPeopleProblem.interviewer.protectedDisclosures[0];
    expect(disclosure).toBeDefined();
    if (disclosure === undefined) throw new Error("missing protected disclosure");
    const reviewedText = disclosure.fact + " What part of that argument needs justification?";

    const validator = new DisclosureValidator(new ClosedWorldDisclosureAnalyzer([reviewedText]));
    const result = validator.validate({
      proposal: {
        realizedAction: "DIRECTIONAL_NUDGE",
        claimedDisclosureLevel: disclosure.minimumDisclosureLevel,
        claimedDisclosureIds: [disclosure.id],
        speechText: reviewedText
      },
      request: {
        requiredAction: "DIRECTIONAL_NUDGE",
        maximumDisclosure: disclosure.minimumDisclosureLevel,
