...
interface SessionMetrics {
  ...
  pendingLocalTimings: Record<LocalTimingKind, number>;
}
...
function emptyMetrics(partial = false): SessionMetrics {
  return {
    ...
    pendingLocalTimings: { STT: 0, TTS: 0, VISION: 0 }
  };
}
...
function decrementBounded(value: number): number {
  return Math.max(0, boundedCount(value) - 1);
}

function formalResultCount(metrics: SessionMetrics): number {
  return addBounded(
    addBounded(
      addBounded(metrics.formal.accepted, metrics.formal.abstentions),
      addBounded(metrics.formal.timeouts, metrics.formal.cancelled)
    ),
    metrics.formal.failedOrMalformed
  );
}
...
function parsePersistedMetrics(value: unknown): SessionMetrics | undefined {
  ...
  try {
    ...
    if (
      !Number.isSafeInteger(metrics.pendingRemote.interviewer)
      || metrics.pendingRemote.interviewer < 0
      || !Number.isSafeInteger(metrics.pendingRemote.formal)
      || metrics.pendingRemote.formal < 0
      || metrics.pendingRemote.interviewer > metrics.remoteTotals.interviewerCalls
      || metrics.pendingRemote.formal > metrics.remoteTotals.formalInterpretationCalls
    ) return undefined;
    const terminalRemoteCount = RemoteReasoningOutcomeSchema.options.reduce(
      (total, outcome) => addBounded(total, metrics.remoteTotals.outcomes[outcome]),
      0
    );
    const startedRemoteCount = addBounded(
      metrics.remoteTotals.interviewerCalls,
      metrics.remoteTotals.formalInterpretationCalls
    );
    const pendingRemoteCount = addBounded(
      metrics.pendingRemote.interviewer,
      metrics.pendingRemote.formal
    );
    if (addBounded(terminalRemoteCount, pendingRemoteCount) !== startedRemoteCount) {
      return undefined;
    }
    ...
    for (const kind of ["STT", "TTS", "VISION"] as const) {
      ...
      if (
        ...
        || !Number.isSafeInteger(metrics.pendingLocalTimings[kind])
        || metrics.pendingLocalTimings[kind] < 0
      ) return undefined;
    }
    return metrics;
  } catch {
...
export class SessionObservability {
  ...
  public beginRemoteOperation(input: {
    ...
    this.mutate(sessionId, (metrics) => {
      if (operation === "INTERVIEWER_REALIZATION") {
        ...
        metrics.pendingRemote.interviewer =
          addBounded(metrics.pendingRemote.interviewer, 1);
      } else {
        metrics.remoteTotals.formalInterpretationCalls =
          addBounded(metrics.remoteTotals.formalInterpretationCalls, 1);
        metrics.pendingRemote.formal =
          addBounded(metrics.pendingRemote.formal, 1);
      }
    });
    return {
      startedAt,
      ...
      finish: (outcome) => {
        ...
        try {
          ...
          this.mutate(sessionId, (metrics) => {
            ...
            if (operation === "INTERVIEWER_REALIZATION") {
              metrics.pendingRemote.interviewer =
                decrementBounded(metrics.pendingRemote.interviewer);
            } else {
              metrics.pendingRemote.formal =
                decrementBounded(metrics.pendingRemote.formal);
            }
  ...
  public recordCandidateSubstantiveTurn(sessionId: SessionId, turnIdInput: string): void {
    ...
    this.increment(sessionId, (metrics) => {
      ...
      if (metrics.candidateTurnIds.length === MAX_CANDIDATE_TURN_IDS) {
        // Once the exact deduplication set is saturated, do not evict identities
        // and continue incrementing: a retry of an evicted turn would fabricate
        // a larger aggregate. Freeze the exact lower bound and mark it partial.
        metrics.partial = true;
        return;
      }
  ...
  public beginLocalTiming(sessionIdInput: SessionId, kind: LocalTimingKind): LocalTimingHandle {
    ...
    let finished = false;
    this.mutate(sessionId, (metrics) => {
      metrics.pendingLocalTimings[kind] =
        addBounded(metrics.pendingLocalTimings[kind], 1);
    });
    return {
      finish: (outcome) => {
        if (finished) return;
        ...
        try {
          ...
          this.mutate(sessionId, (metrics) => {
            metrics.pendingLocalTimings[kind] =
              decrementBounded(metrics.pendingLocalTimings[kind]);
            if (elapsed === null) {
              metrics.partial = true;
            } else {
              const samples = metrics.latencies[kind];
  ...
  private project(metricsInput: SessionMetrics): SessionPerformanceSummary {
    ...
    return SessionPerformanceSummarySchema.parse({
      ...
      partial:
        ...
        || metrics.pendingRemote.interviewer > 0
        || metrics.pendingRemote.formal > 0
        || metrics.pendingLocalTimings.STT > 0
        || metrics.pendingLocalTimings.TTS > 0
        || metrics.pendingLocalTimings.VISION > 0
        || formalResultCount(metrics) < metrics.formal.attempts
        || (
          metrics.local.ttsSuccesses
          + metrics.local.ttsFailures
          + metrics.local.ttsCancellations
          < metrics.local.ttsRequests
        )
        || (
          metrics.local.visionInferenceCompletions
          + metrics.local.visionInferenceFailures
          < metrics.local.visionRequests
        ),
