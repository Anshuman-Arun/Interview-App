...
export class VoiceInputCoordinator {
  ...
  private async applyEvents(
    ...
    let validatedEvents: SpeechWorkerEvent[];
    try {
      if (
      ...
      validatedEvents = events.map((event) => SpeechWorkerEventSchema.parse(event));
    } catch (error) {
      if (context.sttTiming !== undefined) {
    ...
    try {
      ...
      const sttTimingWasActive = context.sttTiming !== undefined;
      if (sttTimingWasActive) {
        context.sttTiming?.finish("FAILURE");
        context.sttTiming = undefined;
      }
